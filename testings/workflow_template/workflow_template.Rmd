...
load.cell.file.pattern <- "^([GCYRT]FP|[GCYRT]\\d+)_Position\\d+.tif$"

# arguments.file.pattern <- "^(BF|TFP)_Position(\\d+)_time(\\d+).tif$"
# load.cell.file.pattern <- "^([GCYRT]FP|[GCYRT]\\d+)_Position\\d+_time\\d+.tif$"
```
  ...
  foreach(test.param=test.params, #.export = c(".parameters.df",)
          ...
          .packages = c("rcell2", "base", "dplyr")) %dopar% {
            ...
            cell.data <- rcell2::cell.load.alt(
              tmp.path,
              fluorescence.pattern = load.cell.file.pattern
              )
