...
```{r}
rcell2::cell2(a, cell.command = "~/Software/cellID-linux/cell", output_coords_to_tsv = T)
```

```{r}
cell.data <- rcell2::cell.load.alt(target.dir)

cell.masks <- rcell2::cell.load.boundaries(target.dir, data.source = 'masks.tsv', arguments = a, pixel.type = c("i", "b"))
```
...
```{r}
X <- with(sample.cords, {
  spatstat.geom::ppp(x, y, window=owin_poly_window)
})
