...
# path <- "~/Projects/Colman/HD/reentry/analisis_Far1NG_2019Dai/data/20191212_Timecourse_FAR1-NG_RtCC/split/"
# parameters <-  paste0(path, "parameters_vcellid_out.txt")
...
```{r}
cell.data <- rcell2::cell.load.alt(path =  path, pdata = path.pdata)
cell.data.old <- rcell2::cell.load.alt(path =  path, pdata = path.pdata)
...
```{r}
setdiff((cell_data.old$data %>% names), (cell_data.new$data %>% names))
```
