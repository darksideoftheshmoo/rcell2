...
```
# Bug: las boundaries parecen "abiertas"

Parece que faltan pixeles en el boundary, sea por que hay dos celulas pegadas o no:
...
p <- masks %>% 
  filter(cellID %in% cids, pos==1, pixtype=="i", channel=="YFP") %>% 
  ggplot() + 
  geom_tile(aes(x=x,y=y,fill=factor(cellID))) + facet_grid(t.frame~flag)
plotly::ggplotly(p)

p <- masks %>% 
  filter(cellID %in% cids, pos==1, pixtype=="b", channel=="YFP") %>% 
  ggplot() + 
  geom_tile(aes(x=x,y=y,fill=factor(cellID))) + facet_grid(t.frame~flag)
plotly::ggplotly(p)
...
### TODO - cosas para probar

De hecho los puntos en el TIFF son puestos por `add_boundary_points_to_data` indirectamente.

Hace una cuentita con ellos para ponerlos en el array `d` (que tiene las "dimensiones" del TIFF).

Hay dos opciones, la primera es que no estoy haciendo el "full circuit", que en esa función dice que se hace con `if(p2==NULL) p2=p_start; //So make full circuit`.

La segunda es que el resto de las cuentitas son una interpolación de los puntos en `cellblob->boundary`.
