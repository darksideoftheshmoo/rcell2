...
Code for this check is available in the template notebook of the `rcell2.magick` package.
...
Execute the following to install the examples package and set the data directory:
...
```{r}
cell.data <- rcell2.cellid::cell.load.alt(path = data_dir)
```
...
n_axis1 <- images %>% filter(!is.out) %>% with(unique(t.frame)) %>% length()
n_axis2 <- images %>% filter(!is.out) %>% with(unique(pos)) %>% length()
n_axis3 <- images %>% filter(!is.out) %>% with(unique(channel)) %>% length()

# Check
stopifnot((n_axis1 * n_axis2 * n_axis3) ==  nrow(unique(filter(images, !is.out))))

macro <- glue::glue(
  '// Macro',
  # 'run("Image Sequence...", "open={data_dir} file=(.*\\\\d.tif$) sort use");',
  'run("Image Sequence...", "open={data_dir} file=(.*out.tif$) sort use");',
...
counter.desc.df <- data.frame(Counter = 0:3,
