...
Run cellid using the selected parameters, and load data into R.
...
```{r}
data.dir <- system.file("extdata/image_samples/", package = "rcell2.cellid")
...
cell2.out <-
  rcell2.cellid::cell2(arguments = cellid.args,
                       # cell.command = "~/Software/cellID-linux/cell"  # Optional
                       # output_coords_to_tsv = T,  # Optional, works only with CellID version >= 1.5
                       )
...
* Imaging channel (birghtfield, transmission, fluorescence, z-level, etc.).
* Microscope position (*pos*).
* Time (optional).

For example, expected file names have this appearance:
...
Later on, the file list has to be recognized by a specific regular expression (regex).
A regex matching these files can be `^(BF|TFP)_Position(\\d+)_time(\\d+).tif$`,
and it might be used by one of the image loading functions in this package.

Always read the help page of each function for details, for example:
...
> These examples require the `rcell2.cellid` package.
...
> Optional section; these scripts below are for demostration purposes.
...
```{r}
parameters.list <- rcell2.cellid::parameters_default(
  # If tou have my `utiles` package, `value.to.range()` may come in handy.
